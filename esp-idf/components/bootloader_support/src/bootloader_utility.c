...
/*  test
    ...
    uint32_t d1[8] = {20,20,20,20,20,20,20,20};
    size_t src_addr = 0x9000;
    
    uint32_t f = 10;
    esp_err_t c = bootloader_flash_read(src_addr,&f,sizeof(uint32_t),false); 
    ESP_LOGD(TAG,"-1 spi_flash_rw,cpu_start: %d\n",f);
    ESP_LOGD(TAG,"0 spi_flash_rw,cpu_start,err: %x\n",c);

    esp_err_t a1 =  bootloader_flash_write(src_addr,d1,32,true); 
    
    ESP_LOGD(TAG,"1 spi_flash_rw,cpu_start,err: %x\n",a1);

    uint32_t t[8];
    esp_err_t b = bootloader_flash_read(src_addr,t,32,true); 
    ESP_LOGD(TAG,"2 spi_flash_rw,cpu_start: %d\n",t[0]);
    ESP_LOGD(TAG,"3 spi_flash_rw,cpu_start,err: %x\n",b);
*/
...
/* **** MOD. ***** */
    uint8_t cnt = (data -> image).segment_count;
    for(uint8_t i=0;i<cnt;++i){
        esp_image_segment_header_t segment_header = (data->segments)[i];
        uint32_t data_addr = data->segment_data[i];
        uint32_t load_addr = segment_header.load_addr;
        uint32_t len = segment_header.data_len;
        ESP_LOGV(TAG, "segment %d : paddr=%08x vaddr=%08x size=%08x", i, data_addr, load_addr, len);
    }

    ESP_LOGD(TAG,"_iram_text_start: 0x%08x",(int)(&_iram_text_start));
    ESP_LOGD(TAG,"_iram_text_end: 0x%08x",(int)(&_iram_text_end));
    
    struct _iram_text_address{
        int start;
        int end;
    };
    struct _iram_text_address addr;
    addr.start = (int)&_iram_text_start;
    addr.end = (int)&_iram_text_end;

    struct _iram_text_address *addrptr = (struct _iram_text_address *)0x3FFFFFF0;
    *addrptr = addr;
    ESP_LOGD(TAG,"start,end: %x,%x\n",addrptr->start,addrptr->end);
/* **** MOD. ***** */
