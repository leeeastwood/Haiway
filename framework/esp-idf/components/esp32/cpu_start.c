...
void vGetCodeSegmentsHash(unsigned char digest[16], codeSegs *segs){
	...
	for(uint8_t i=0;i<cnt;++i){
        ...
        for(uint32_t j=start_addr;j<end_addr;j+=4){
			mbedtls_md5_update_ret(&context,(unsigned char *)j,4);
        }
    }

	mbedtls_md5_finish_ret(&context,digest);
}

void codeSegsInit(codeSegs *segs){

    struct _iram_text_address{
        int start;
        int end;
    };
    struct _iram_text_address *addrptr = (struct _iram_text_address *)0x3FFFFFF0;
    // printf("Hello World! %08x,%08x\n",addrptr->start,addrptr->end);

	segs->cnt = 1;

	segs->seg[0].start_addr = addrptr->start;
	segs->seg[0].len = addrptr->end - addrptr->start;

	vGetCodeSegmentsHash(segs->digest, segs);
...
void vCheckCodeSegments(){
    ...
    struct start_res_t *start_res = (struct res_t *)0x3FFFFFF0;
    start_res->recv_cnt = start_res->send_cnt = 0;

	while(true){
		unsigned char digest[16];

        // printf("recv_cnt,send_cnt: %d, %d\n",start_res->recv_cnt,start_res->send_cnt);

		vGetCodeSegmentsHash(digest, &segs);

        printf("-----Safe Model-----\n");
        printf(".text start: %08x\n",addrptr->start);
        printf(".text end: %08x\n",addrptr->end);
        printf("standard digest:\n");
        for(int i=0;i<16;++i){
            printf("%d ",segs.digest[i]);
		} 
        printf("\n");
        printf("digest:\n");
        for(int i=0;i<16;++i){
...
#if( taskKERNEL_SAFE > 0 )
    portBASE_TYPE   res = xTaskCreatePinnedToCore(&vCheckCodeSegments, "vCheckCodeSegments",
                                                    CONFIG_MAIN_TASK_STACK_SIZE, NULL,
                                                    ESP_TASK_PRIO_MAX, NULL, 0);
    assert(res == pdTRUE);
#endif 
