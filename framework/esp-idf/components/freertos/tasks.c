...
#if( taskKERNEL_SAFE > 0 )

#include "MD5.h"

// #define portYIELD_WITHIN_API() vCheckCodeSegments(),portYIELD_WITHIN_API()

#define MAX_SEGMENTS_COUNT 10
...
typedef struct codeSegments{
	uint32_t cnt;
	codeSeg seg[MAX_SEGMENTS_COUNT];
	unsigned char digest[16];
}codeSegs;

void vGetCodeSegmentsHash(unsigned char digest[16]){
	uint8_t cnt = 1;

	MD5_CTX context;

	char j[] = "china";
	MD5Update(&context,(unsigned char *)j,6);

	// for(uint8_t i=0;i<cnt;++i){
    //     uint32_t start_addr = segs.seg[i].start_addr;
    //     uint32_t len = segs.seg[i].len;

    //     for(uint32_t j=start_addr;j<start_addr+len;j+=4){
	// 		MD5Update(&context,(unsigned char *)j,4);
    //     }
    // }
...
void codeSegsInit(codeSegs segs){
	segs.cnt = 9;
	segs.seg[0].start_addr = 0x3f400020;
	segs.seg[0].len = 21916;

	segs.seg[1].start_addr = 0x3ffb0000;
	segs.seg[1].len = 8856;
	
	segs.seg[2].start_addr = 0x3ffb2298;
	segs.seg[2].len = 0;

	segs.seg[3].start_addr = 0x40080000;
	segs.seg[3].len = 1024;

	segs.seg[4].start_addr = 0x40080400;
	segs.seg[4].len = 33692;

	segs.seg[5].start_addr = 0x400d0018;
	segs.seg[5].len = 72868;

	segs.seg[6].start_addr = 0x4008879c;
	segs.seg[6].len = 836;
	
	segs.seg[7].start_addr = 0x400c0000;
	segs.seg[7].len = 0;

	segs.seg[8].start_addr = 0x50000000;
	segs.seg[8].len = 0;
	...
	vGetCodeSegmentsHash(segs.digest);	
}

void vCheckCodeSegments(){
	codeSegs segs;
	codeSegsInit(segs);
	while(true){
		unsigned char digest[16] = {0};
		vGetCodeSegmentsHash(digest);

		int err = 0;
		for(int i=0;i<16;++i){
			if(digest[i] != segs.digest[i]){
				err = 1;
				break;
			}
		}
		...
		if(err){
			printf("code segments is changed!!!");
		}
		else{
			printf("code segments check pass!");
		}
	...
	#define taskYIELD_IF_USING_PREEMPTION()
		...
		#define taskYIELD_IF_USING_PREEMPTION() vCheckCodeSegments(),portYIELD_WITHIN_API()
	#else
...
/*-----------------------------------------------------------*/
	...
	BaseType_t xReturn;
		...
		#if( portSTACK_GROWTH <= 0 )
		{
			...
			StackType_t *pxStack = ( StackType_t * ) pvPortMallocStackMem( ( sizeof( StackType_t ) * ( ( size_t ) usStackDepth ) ) ); /*lint !e961 MISRA exception as the casts are only redundant for some ports. */
		...
		#else /* portSTACK_GROWTH */
		{
			...
			if( pxNewTCB != NULL )
			{
				...
				pxNewTCB->pxStack = ( StackType_t * ) pvPortMallocStackMem( ( sizeof( StackType_t ) * ( ( size_t ) usStackDepth ) ) ); /*lint !e961 MISRA exception as the casts are only redundant for some ports. */
...
/*-----------------------------------------------------------*/
	...
	BaseType_t xAlreadyYielded = pdFALSE;
		...
		TickType_t TickType_t_0u = ( TickType_t ) 0U;
		if( TickType_t_0u >= xTicksToDelay )
		{
			mtCOVERAGE_TEST_MARKER();
		}
		else
...
//			vTaskSuspendAll();
			{
				UBaseType_t UBaskType_t_0 = ( UBaseType_t ) 0;
				traceTASK_DELAY();
...
//				portTICK_TYPE_EXIT_CRITICAL( &xTickCountMutex );
				...
				both lists. */
				if( uxListRemove( &( pxCurrentTCB[ xPortGetCoreID() ]->xGenericListItem ) ) == UBaskType_t_0 )
				{
					/* The current task must be in a ready list, so there is
					no need to check, and the port reset macro can be called
					directly. */
...
BaseType_t xReturn;
BaseType_t i;
	//printf("-----start esp-idf scheduler-----\n");		

	#if( taskKERNEL_SAFE == 2 )
		printf("taskKERNEL_SAFE: vTaskStartScheduler!!!\n");		
		xTaskCreate(&vCheckCodeSegments, "vCheckCodeSegments",
